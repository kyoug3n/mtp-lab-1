...
- [1. Изменение файла и второй коммит (средняя №4)](#1-изменение-файла-и-второй-коммит-средняя-4)
- [2. Слияние ветки feature с основной (средняя №6)](#2-слияние-ветки-feature-с-основной-средняя-6)
- [3. Клонирование чужого репозитория и разбор истории (средняя №10)](#3-клонирование-чужого-репозитория-и-разбор-истории-средняя-10)
- [4. Переписывание истории с `git rebase` (повышенная №5)](#4-переписывание-истории-с-git-rebase-повышенная-5)
- [5. Отчёт о коммитах с `git shortlog` (повышенная №9)](#5-отчёт-о-коммитах-с-git-shortlog-повышенная-9)
...
```
test_average (tests.test_text_stats.AverageWordLengthTest.test_average) ... ok
test_empty_text_returns_zero (tests.test_text_stats.AverageWordLengthTest.test_empty_text_returns_zero) ... ok
test_counts_words (tests.test_text_stats.WordCountTest.test_counts_words) ... ok
test_empty_text (tests.test_text_stats.WordsTest.test_empty_text) ... ok
test_splits_and_lowercases (tests.test_text_stats.WordsTest.test_splits_and_lowercases) ... ok
...
| Ветки | копия каталога на сервере (`svn copy`): создаётся дёшево, но ветвление и слияние идут через сервер | указатель на коммит в локальном репозитории, создаётся и сливается без сервера |
